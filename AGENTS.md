...
- **Canonical skills dir:** `.agents/skills/`. `.claude/skills/`, `.cursor/skills/`, and `.codex/skills/` symlink into it (`.cursor/skills/web-design-guidelines` and `.codex/skills/web-design-guidelines` are real dirs, not symlinks). Add skills under `.agents/skills/`, not the symlinked dirs.
...
- **Generated code, don't hand-edit:** `src/routeTree.gen.ts` (regenerated by `vite dev` / `vite build` from the routes dir) and `worker-configuration.d.ts` (checked in; regenerate with `bun run cf:typegen` / `wrangler types` after changing `wrangler.jsonc`, e.g. adding a binding).
- **Design system source of truth is `DESIGN.md`** at the repo root, not the frontend README.

### Stale Next.js artifacts (do not trust or follow)

- `.eslintrc.json` references `next/core-web-vitals` and `next/typescript`
- `.gitignore` references `.next/`
