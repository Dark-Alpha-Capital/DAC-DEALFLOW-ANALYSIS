...
import { investorLeadStatusLabels } from "../columns";
...
async function CachedInvestorLeadContent({ uid }: { uid: string }) {
  "use cache";
  cacheTag(`investor-lead-${uid}`);
  cacheLife("hours");

  let data: Awaited<ReturnType<typeof GetInvestorLeadWithRelations>> | null =
    null;
  let error: Error | null = null;

  try {
    data = await GetInvestorLeadWithRelations(uid);
  } catch (err) {
      ...
      <div className="mb-8 flex items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <Button asChild size="sm" variant="ghost" className="shrink-0">
            <Link href={`/investor-leads/${lead.id}/edit`} className="gap-2">
          ...
          {existingInvestor ? (
            <>
              ...
              <Button asChild size="sm" variant="outline">
                <Link href={`/investors/${existingInvestor.id}`}>
                  View Investor
                </Link>
              </Button>
            </>
            ...
            <Button asChild size="sm">
              <Link href={`/investor-leads/${lead.id}/convert`}>
                Convert to Investor
              </Link>
            </Button>
          )}
...
async function AuthedInvestorLeadContent(props: {
  params: Params;
  ...
  return <CachedInvestorLeadContent uid={params.uid} />;
}

export default function InvestorLeadDetailPage(props: { params: Params }) {
  const sessionPromise = getSession();
  return (
    <Suspense fallback={<InvestorLeadPageSkeleton />}>
      <AuthedInvestorLeadContent
        params={props.params}
