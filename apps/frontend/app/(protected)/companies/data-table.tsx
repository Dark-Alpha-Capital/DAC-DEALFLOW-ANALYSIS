...
import type { CompanyWithTheme } from "./columns";
import {
  Table,
  TableBody,
  TableCell,
...
export function CompaniesDataTable({ columns, data }: CompaniesDataTableProps) {
  ...
  return (
    <div className="w-full">
      <div className="flex flex-col gap-3 py-4 sm:flex-row sm:items-center sm:gap-4">
        ...
        <Select
          value={
            (table.getColumn("coverageStatus")?.getFilterValue() as string) ??
            "all"
          }
          onValueChange={(v) =>
            table
              .getColumn("coverageStatus")
