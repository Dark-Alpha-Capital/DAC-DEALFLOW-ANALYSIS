...
async function CachedEditContent({ uid }: { uid: string }) {
  ...
  try {
    company = await GetCompanyById(uid);
  } catch (err) {
    ...
    return (
      <section className="flex min-h-[60vh] items-center justify-center px-4">
        <div className="border-border w-full max-w-md space-y-4 border-b pb-8 text-center">
          <h1 className="text-foreground text-xl font-semibold">Error loading company</h1>
          <Button asChild>
  ...
  if (!company) {
    return (
      <section className="flex min-h-[60vh] items-center justify-center px-4">
        <div className="border-border w-full max-w-md space-y-4 border-b pb-8 text-center">
          <h1 className="text-foreground text-xl font-semibold">Company not found</h1>
          <Button asChild>
            <Link href="/companies">Back to Companies</Link>
          </Button>
        </div>
...
export default function EditCompanyPage(props: { params: Params }) {
