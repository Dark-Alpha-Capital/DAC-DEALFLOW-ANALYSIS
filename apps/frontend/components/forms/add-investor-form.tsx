...
const AddInvestorFormSchema = z.object({
...
type AddInvestorFormSchemaType = z.infer<typeof AddInvestorFormSchema>;

export default function AddInvestorForm() {
  const router = useRouter();
  const trpc = useTRPC();

  const { mutate: createInvestor, isPending } = useMutation(
    trpc.investors.create.mutationOptions({
      onSuccess: (data) => {
  ...
  const form = useForm<AddInvestorFormSchemaType>({
    resolver: zodResolver(AddInvestorFormSchema),
    defaultValues: {
      name: "",
      type: "HNWI",
      primaryContactName: "",
  ...
  function onSubmit(values: AddInvestorFormSchemaType) {
    createInvestor({
      ...values,
