...
const AddInvestorLeadFormSchema = z.object({
...
type AddInvestorLeadFormSchemaType = z.infer<typeof AddInvestorLeadFormSchema>;

export default function AddInvestorLeadForm() {
  const router = useRouter();
  const trpc = useTRPC();

  const { mutate: createLead, isPending } = useMutation(
    trpc.investorLeads.create.mutationOptions({
      onSuccess: (data) => {
      ...
      onError: (error) => {
        toast.error(error.message || "Failed to add investor lead");
      },
    }),
  );

  const form = useForm<AddInvestorLeadFormSchemaType>({
    resolver: zodResolver(AddInvestorLeadFormSchema),
    defaultValues: {
      name: "",
      source: "",
      email: "",
  ...
  function onSubmit(values: AddInvestorLeadFormSchemaType) {
    createLead(values);
  }
        ...
        <FieldSet>
          ...
          <div className="mt-6 flex gap-3">
            ...
            <Button type="submit" disabled={isPending}>
              {isPending ? "Saving..." : "Add Investor Lead"}
            </Button>
          </div>
        </FieldSet>
      </form>
