...
export function InteractionsFeed({
  ...
  entity: "investor" | "investorLead";
  entityId: string;
  initialInteractions: InteractionRow[];
}) {
  const router = useRouter();
  const trpc = useTRPC();
  // Both routers expose identical interaction procedures; the union cast keeps the
  // implementation in one place. Typed as the investors router since the shapes match.
  const interactionsApi = (entity === "investor"
    ? trpc.investors
    : trpc.investorLeads) as typeof trpc.investors;
  ...
  const { mutate: createInteraction, isPending: creating } = useMutation(
    interactionsApi.createInteraction.mutationOptions({
      onSuccess: async () => {
  ...
  const { mutate: updateInteraction, isPending: updating } = useMutation(
    interactionsApi.updateInteraction.mutationOptions({
      onSuccess: async () => {
  ...
  const { mutate: deleteInteraction, isPending: deleting } = useMutation(
    interactionsApi.deleteInteraction.mutationOptions({
      onSuccess: async () => {
  ...
  function onSubmit(values: InteractionFormValues) {
    ...
    const createInput = {
      ...
      ...(entity === "investor"
        ? { investorId: entityId }
        : { investorLeadId: entityId }),
    } as Parameters<typeof createInteraction>[0];

    const updateInput = {
      ...createInput,
      id: editing!.id,
    } as Parameters<typeof updateInteraction>[0];

    if (editing) {
      updateInteraction(updateInput);
    } else {
      createInteraction(createInput);
    }
  }
              ...
              onDelete={(item) => {
                deleteInteraction({
                  ...(entity === "investor"
                    ...
                    : { investorLeadId: entityId }),
                  id: item.id,
                } as Parameters<typeof deleteInteraction>[0]);
              }}
            />
          ))}
        </ol>
