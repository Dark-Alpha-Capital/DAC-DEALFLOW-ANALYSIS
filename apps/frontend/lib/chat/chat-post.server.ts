...
import { coerceStoredMessages } from "@/lib/chat/chat-messages";
import {
...
} from "@/lib/chat/chat-diligence-tools";
...
export async function postChat(req: Request): Promise<Response> {
  ...
  try {
    ...
    const stream = streamText({
      ...
      instructions: buildFullChatSystemPrompt(),
    ...
    const uiStream = toUIMessageStream({
      ...
      onEnd: async ({
        ...
        try {
          ...
          console.error("[chat] onEnd saveChatSessionMessages failed:", err);
        }
        if (isAborted) {
          return;
        }
