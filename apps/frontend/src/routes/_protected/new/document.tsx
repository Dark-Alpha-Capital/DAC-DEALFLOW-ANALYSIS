import { createFileRoute } from "@tanstack/react-router";
import { FileText } from "lucide-react";

export const Route = createFileRoute("/_protected/new/document")({
  head: () => ({
    meta: [{ title: "Document — Dark Alpha Capital" }],
  }),
  component: DocumentPlaceholderRoute,
});

function DocumentPlaceholderRoute() {
  return (
    <div className="big-container block-space">
      <div className="flex flex-col items-center justify-center rounded-lg border border-dashed py-16 text-center">
        <FileText className="text-muted-foreground mb-4 h-12 w-12" />
        <h2 className="text-lg font-semibold">Document analysis</h2>
        <p className="text-muted-foreground mt-2 max-w-sm text-sm">
          PDF deal extraction has been removed. Use the deals pipeline or manual
          entry to add deals.
        </p>
      </div>
    </div>
  );
}
